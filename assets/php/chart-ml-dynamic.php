...
function makeMonthlyMLChart() {
    try {
        global $conn;

        // Получить данные по месяцам и рейтингу
        $stmt = $conn->prepare("SELECT DATE_FORMAT(created_at, '%Y-%m') AS month, rating, COUNT(*) as count FROM knowledge WHERE knowledge_type = \"ML\" AND created_at >= DATE_SUB(NOW(), INTERVAL 1 YEAR) GROUP BY month, rating ORDER BY month ASC");
        $stmt->execute();
